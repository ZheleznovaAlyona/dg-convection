...
namespace solver
{
	MyVector GMRES::solve(MyVector& U_begin, double &normL2u, slae::SLAE& slae_in, Logger& my_logger)
	{
		...
		int k_iter = 1;
		for(k_iter = 1; k_iter <= s_parameters.max_number_of_iterations && norm_r / norm_f > s_parameters.epsilon; k_iter++)
		{
			d.make_zero();
			V[0] = r / norm_r;
		...
		//my_logger.send_current_information_to_screen((slae_in.A * x - f).norm() / norm_f, k_iter);
		return x;
	}
